...
module.exports = {
    ...
    onStart: async function ({ api, event, args, usersData }) {
        try {
            ...
            if (args[0] === 'remove') {
                const dat = (await axios.get(`${link}?remove=${encodeURIComponent(dipto.replace("remove ", ""))}&senderID=${uid}`)).data.message;
            ...
            if (args[0] === 'list') {
                if (args[1] === 'all') {
                    ...
                    const limited = data?.teacher?.teacherList?.slice(0, 100) || [];
                    const teachers = await Promise.all(limited.map(async (item) => {
                        const number = Object.keys(item)[0];
                        const name = await usersData.getName(number).catch(() => number) || "Not found";
                        return { name, value: item[number] };
                    }));
                    teachers.sort((a, b) => b.value - a.value);
                    return api.sendMessage(`Total Teach = ${data.length}\n👑 | List of Teachers\n${teachers.map((t, i) => `${i + 1}/ ${t.name}: ${t.value}`).join('\n')}`, event.threadID, event.messageID);
                } else {
            ...
            if (args[0] === 'msg') {
                const d = (await axios.get(`${link}?list=${encodeURIComponent(dipto.replace("msg ", ""))}`)).data.data;
                return api.sendMessage(`Message = ${d}`, event.threadID, event.messageID);
            }
            ...
            if (args[0] === 'teach') {
                const isAmar = args[1] === 'amar';
                const isReact = args[1] === 'react';
                const [comd, command] = dipto.split(/\s*-\s*/);
                const final = comd.replace(isReact ? "teach react " : "teach ", "");
                if (!command || command.length < 2) return api.sendMessage('❌ | Invalid format!', event.threadID, event.messageID);
                
                let url = `${link}?teach=${encodeURIComponent(final)}&reply=${encodeURIComponent(command)}&senderID=${uid}`;
                if (isReact) url = `${link}?teach=${encodeURIComponent(final)}&react=${encodeURIComponent(command)}`;
                if (isAmar) url = `${link}?teach=${encodeURIComponent(final)}&senderID=${uid}&reply=${encodeURIComponent(command)}&key=intro`;
                else url += `&threadID=${event.threadID}`;

                const re = (await axios.get(url)).data;
                return api.sendMessage(`✅ Replies added ${re.message}`, event.threadID, event.messageID);
            }

            if (/amar name ki|amr nam ki|amar nam ki|amr name ki|whats my name/.test(dipto)) {
    ...
    onChat: async function ({ api, event }) {
        try {
            ...
            const customTriggers = {
                ...
                "আসসালামু আলাইকুম": ["ওয়ালাইকুম আসসালাম ওয়া রহমতুল্লাহ্ জান", "ওয়ালাইকুম আসসালাম, কেমন আছো বলো?", "Slm r rpl dila bby hba nki? 🙈"],
                ...
                "নাম কি": ["আমার নাম 𓆩»̶̶͓͓͓̽̽̽𝆠꯭፝֟ɴɪᴊʜᴜᴍ-ᴄʜᴀᴛ-ʙᴏᴛ𝆠꯭፝֟⚜️𓆪, আর আমার বসের নাম তো জানেনই 👑𝆠፝𝐒𝐈𝐘𝐀𝐌-𝐇𝐀𝐒𝐀𝐍 👑", "সবাই ভালোবেসে জানু ডাকে 🙈"],
            ...
            if (/^(baby|bby|bot|jan|babu|janu)/.test(body)) {
                ...
                if (!arr) {
                    const randomReplies = [
                        ...
                        "ʙᴀʙʏ, ᴀᴍɪ ᴛᴏᴍᴀʀ ᴏᴘᴇᴋʜʏᴀʏ 🇨🇮ʟᴀᴍ 💖", "𝗵𝗲 𝗯𝗼𝘁 𝗯𝗼𝘁 𝗰𝗵𝗶𝗹𝗹 𝗯𝗿𝗼!", "I love you 💝", "আমি  𓆩👑𝆠፝𝐒𝐈𝐘𝐀𝐌-𝐇𝐀𝐒𝐀𝐍👑𓆪 বস এর সাথে বিজি আছি-😕😏",
                        "আমার বস 𓆩👑𝆠፝𝐒𝐈𝐘𝐀𝐌-𝐇𝐀𝐒𝐀𝐍👑𓆪 কে একটা জি GF দাও-😽🫶", "জান তোমার নানি রে আমার হাতে তুলে দিবা-🙊🙆‍♂",
                        "𓆩👑𝆠፝𝐒𝐈𝐘𝐀𝐌-𝐇𝐀𝐒𝐀𝐍👑𓆪 বস'এর হবু বউ রে কেও দেকছো?😪", "জান হাঙ্গা করবা-🙊😝", "ইসস এতো ডাকো কেনো লজ্জা লাগে তো-🙈🖤", "তাকাই আছো কেন চুমু দিবা-🙄🐸😘",
